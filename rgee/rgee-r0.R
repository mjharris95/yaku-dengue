...
aedes_r0 <- read.csv("D:/Attribution/AedesR0Out.csv")
...
for(i in failed_units){
  ...
  daily <-sapply(days, function(day) {
    ...
    filtered_transform <-  filtered_round$map(function(image){
    ...
    filtered_mean <- filtered_transform$mean()
  ...
  export_temp_task <- ee_table_to_drive(
    collection = daily,
    description = paste0(country,'_adm1_daily-r0-', i),
    fileFormat = 'CSV',
